...
function install_client() {
    ...
    # 请输入服务端地址（支持IP或域名）
    read -p "请输入服务端地址（IP或域名）：" server_address
    # 判断服务端地址是否为空
    if [ -z "$server_address" ]; then
        exception_log "服务端地址不能为空。"
        exit 1
    fi

    # 验证输入是否为合法的IP或域名格式
    if ! [[ $server_address =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]] && ! [[ $server_address =~ ^([a-zA-Z0-9]([a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])?\.)+[a-zA-Z]{2,}$ ]]; then
        # 如果既不是IP也不是域名格式，尝试解析域名
        if ! host "$server_address" >/dev/null 2>&1; then
            exception_log "服务端地址格式错误或无法解析: $server_address"
            exit 1
        fi
    fi
    ...
    if ! [[ "$server_port" =~ ^[0-9]+$ ]] || [ "$server_port" -lt 1 ] || [ "$server_port" -gt 65535 ]; then
        exception_log "端口号必须是1-65535之间的数字"
        exit 1
    fi
    ...
    # 执行npc安装
    echo "正在安装客户端..."
    sudo "$client_path"/npc install -server="$server_address:$server_port" -vkey="$server_vkey" > /dev/null 2>&1
...
# 检查host命令是否存在，如果不存在则安装
if ! command -v host >/dev/null 2>&1; then
    if command -v apt-get >/dev/null 2>&1; then
        sudo apt-get update && sudo apt-get install -y bind9-host
    elif command -v yum >/dev/null 2>&1; then
        sudo yum install -y bind-utils
    elif command -v dnf >/dev/null 2>&1; then
        sudo dnf install -y bind-utils
    else
        echo "无法安装host命令，请手动安装bind-utils或bind9-host包"
    fi
fi
